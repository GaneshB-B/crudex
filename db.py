...
UE_SQL_PORT = quote(SQL_PORT)
UE_SQL_DATABASE = quote(SQL_DATABASE)

# SQL DB URL Format = "dialect+driver://username:password@host:port/database"

# Default Driver
# SQLALCHEMY_DB_URL = f"{UE_SQL_DIALECT}://{UE_SQL_USERNAME}:{UE_SQL_PASSWORD}@{UE_SQL_HOST}:{UE_SQL_PORT}/{UE_SQL_DATABASE}"

# PyMySQL Driver
SQLALCHEMY_DB_URL = f"{UE_SQL_DIALECT}+{UE_SQL_DRIVER}://{UE_SQL_USERNAME}:{UE_SQL_PASSWORD}@{UE_SQL_HOST}:{UE_SQL_PORT}/{UE_SQL_DATABASE}"
